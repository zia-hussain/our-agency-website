...
import { Plus, CreditCard as Edit, Trash2, Eye, Star, Search, Filter, Grid2x2 as Grid, List, FileText, MapPin, Calendar, Building, CheckCircle, AlertCircle, Globe, Award } from 'lucide-react';
...
const PortfolioManagement: React.FC = () => {
  ...
  const handleDelete = async (id: string, title: string) => {
    ...
    try {
      ...
      await logActivity('delete', 'portfolio_projects', id, title);
      refetch();
    } catch (err) {
  ...
  const toggleFeatured = async (id: string, currentFeatured: boolean, title: string) => {
    ...
    try {
      ...
      await logActivity('update', 'portfolio_projects', id, title, { featured: !currentFeatured });
      refetch();
    } catch (err) {
  ...
  const toggleActive = async (id: string, currentActive: boolean, title: string) => {
    if (!isSupabaseConfigured || !supabase) return;
    ...
    try {
      ...
      await logActivity('update', 'portfolio_projects', id, title, { is_active: !currentActive });
      refetch();
    } catch (err) {
      alert('Failed to update project: ' + (err instanceof Error ? err.message : 'Unknown error'));
    }
  ...
  const filteredProjects = projects.filter(project => {
    ...
    return matchesSearch && matchesFeatured && matchesCategory && matchesType;
  });
    ...
    <div className="space-y-8">
      ...
      <div className="bg-card border border-border rounded-xl p-6">
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
          <div>
            <h1 className="text-3xl font-bold text-foreground mb-2">Portfolio & Case Studies Management</h1>
            <p className="text-muted-foreground">
              Manage your project case studies and client success stories with full SEO optimization
            </p>
            <div className="flex items-center gap-4 mt-3 text-sm">
              ...
              <span className="text-muted-foreground">
                Active: <span className="font-semibold text-primary">{projects.filter(p => p.is_active).length}</span>
              </span>
            </div>
          </div>
          ...
          {filteredProjects.map((project, index) => (
            ...
            >
              ...
              <div className="relative aspect-[16/9] overflow-hidden">
                <img
                  src={project.featured_image || 'https://images.pexels.com/photos/7688336/pexels-photo-7688336.jpeg?auto=compress&cs=tinysrgb&w=800'}
                  alt={project.title}
              ...
              <div className="p-6">
                ...
                <div className="flex gap-2">
                  ...
                  <Link to={`/admin/portfolio/edit/${project.id}`} className="flex-1">
                  ...
                  <button
                    onClick={() => toggleActive(project.id, project.is_active, project.title)}
                    className={`p-2 rounded-lg text-sm transition-colors duration-150 ${
                      project.is_active 
        ...
        >
          ...
          <p className="text-muted-foreground mb-6 max-w-md mx-auto">
            {searchTerm || filterFeatured !== 'all' || filterCategory !== 'all'
              ...
              : 'Start by adding your first case study to showcase your expertise'
            }
          </p>
          {(!searchTerm && filterFeatured === 'all' && filterCategory === 'all') && (
            <Link to="/admin/portfolio/new">
