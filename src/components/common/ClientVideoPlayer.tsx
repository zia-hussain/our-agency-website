...
// The premium video-proof treatment, extracted from Home's ClientVideoMoment
// so any page can drop in a real client video with the same cinematic
// frame, custom controls, and playback behavior — instead of a plain
// [rectangle + play icon]. Caption/attribution intentionally live outside
// this component: it only knows how to play a video file well, so a
// caller can swap the source and attribution independently later without
// touching the player itself.
const ClientVideoPlayer: React.FC<ClientVideoPlayerProps> = ({ src, className = "" }) => {
  ...
  return (
    <motion.div
      ...
      className={`relative rounded-3xl overflow-hidden border border-primary/15 shadow-[0_30px_80px_-30px_rgba(0,0,0,0.6)] ring-1 ring-inset ring-white/[0.03] ${className}`}
    >
